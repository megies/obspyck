...
class ObsPyck(QtGui.QMainWindow):
    ...
    def on_qToolButton_showMap_toggled(self):
        ...
        if state:
            ...
            print "%s %.6f %.6f %.4f %.6f %.6f %.4f %.6f %.2f" % (self.dictOrigin['Time'], self.dictOrigin['Longitude'], self.dictOrigin['Latitude'], self.dictOrigin['Depth'], self.dictOrigin['Longitude Error'], self.dictOrigin['Latitude Error'], self.dictOrigin['Depth Error'], self.dictOrigin['Standarderror'], self.dictMagnitude['Magnitude'])
        else:
            self.delEventMap()
            self.fig.clear()
            self.drawAxes()
